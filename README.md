...
| 목적                   | 예시 코드                                         |   |                          |   |                                                       |
| -------------------- | --------------------------------------------- | - | ------------------------ | - | ----------------------------------------------------- |
| **변경된 데이터 있을 때만 저장** | ```js\nif (ds.getCaseCount("U") > 0           |   | ds.getCaseCount("I") > 0 |   | ds.getCaseCount("D") > 0) {\n    this.fnSave();\n}``` |
| **그리드 셀 강제 편집**      | `js\ngr.setCellPos(1);\ngr.showEditor(true);` |   |                          |   |                                                       |
| **콤보 값 읽기**          | `cmbGrade.value // 실제 코드값`                    |   |                          |   |                                                       |

---

필요하면 **카테고리별 정렬**, **파일로 분리**, **README 스타일 꾸미기**, **아이콘 추가 버전**, **영문 버전**도 만들어줄게.
